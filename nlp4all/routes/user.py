...
UserRouter.route("/account", methods=["GET", "POST"])(fresh_login_required(UserController.account))
UserRouter.route("/login", methods=["GET", "POST"])(UserController.login)
UserRouter.route("/reauth", methods=["GET", "POST"])(login_required(UserController.reauth))
