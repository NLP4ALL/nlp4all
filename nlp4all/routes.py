...
from nlp4all.forms import (
    ...
    AddTweetCategoryForm,
    AddBayesianRobotForm,
    TagButton,
    AddBayesianRobotFeatureForm,
    BayesianRobotForms,
    CreateMatrixForm,
    ThresholdForm,
    AnnotationForm,
...
def add_project():
    ...
    if form.validate_on_submit():
        ...
        project = nlp4all.utils.add_project(
...
def project():
    ...
    if form.validate_on_submit():
        ...
        if form.shared.data:
            tweets_by_cat = {
                cat: [t.id for t in project.tweets if t.category == cat.id]
...
def analysis():
    ...
    if "tag" in request.form.to_dict():
        ...
        db.session.commit()
        tag = TweetTag(
            category=category.id, analysis=analysis.id, tweet=the_tweet.id, user=current_user.id
        )
        db.session.add(tag)
        db.session.commit()
    ...
    categories = TweetTagCategory.query.filter(
        TweetTagCategory.id.in_([p.id for p in project.categories])
    ...
    if analysis.annotate == 2:
        ...
        ann_tags = TweetAnnotation.query.filter(
            TweetAnnotation.annotation_tag.in_(ann_names),
    ...
    if not analysis.shared:
        user_analysis_robots = BayesianRobot.query.filter(
            BayesianRobot.user == current_user.id, BayesianRobot.analysis == analysis.id
        ).all()
        if len(user_analysis_robots) == 0:
            robot = BayesianRobot(
                name=current_user.username + "s robot", analysis=analysis.id, user=current_user.id
            )
            db.session.add(robot)
            db.session.flush()
            db.session.commit()
    ...
    return render_template(
        "analysis.html", analysis=analysis, tag_list=tag_list, tweet=the_tweet, form=form, **data
    )
...
def matrix(matrix_id):
    ...
    if form.validate_on_submit():
        ...
        if form.shuffle.data:
            ...
            incl_tweets = sorted(
                [
                    t
                    for t in matrix.matrix_data.items()
                    if t[1]["probability"] >= matrix.threshold and t[1]["class"] != "undefined"
                ],
            ...
            excl_tweets = sorted(
                [
                    t
                    for t in matrix.matrix_data.items()
                    if t[1]["probability"] < matrix.threshold or t[1]["class"] == "undefined"
                ],
                key=lambda x: x[1]["probability"],
                reverse=True,
            )
            ...
            # [str('Pred_'+str(cat_names[c])+"_Real_"+str(cat_names[c-1])) for c in range(len(cat_names))]+[str('Pred_'+str(cat_names[c])+"_Real_"+str(cat_names[c])) for c in range(len(cat_names))]
            class_list_all = []
            # [str('Pred_'+str(cat_names[c])+"_Real_"+str(cat_names[c-1])) for c in range(len(cat_names))]+[str('Pred_'+str(cat_names[c])+"_Real_"+str(cat_names[c])) for c in range(len(cat_names))]
            class_list_all = []
            for c in cat_names:
    ...
    metrics = sorted(
        [t for t in matrix.data["metrics"].items()], key=lambda x: x[1]["recall"], reverse=True
    )
        ...
        id_c = [
            {
                ...
                for k, v in matrix.matrix_data.items()
                if v["real_cat"] == cm and v["probability"] >= matrix.threshold
            }
        ][0]
        ...
        id_c = [
            {
                ...
                for k, v in matrix.matrix_data.items()
                if v["class"] == cm and v["probability"] >= matrix.threshold
            }
        ][0]
        tweets = Tweet.query.filter(Tweet.id.in_(id_c.keys())).all()
    ...
    id_c = [
        {
            ...
            for k, v in matrix.matrix_data.items()
            if v["probability"] >= matrix.threshold and v["class"] != "undefined"
        }
    ][0]
    ...
    id_c = [
        {
            ...
            for k, v in matrix.matrix_data.items()
            if v["probability"] < matrix.threshold or v["class"] == "undefined"
        }
    ][0]
...
def matrix_overview():
    ...
    matrix_info = sorted(
        [t for t in matrix_info.items()], key=lambda x: x[1]["accuracy"], reverse=True
    )
...
def aggregate_matrix():
    ...
    for m in range(n):
        ...
        # select a new
        tnt_list = [
            x for x in list(range(0, len(matrix.training_and_test_sets))) if x not in used_tnt_sets
        ]
        ...
        metrix = sorted(
            [t for t in new_mx.data["metrics"].items()], key=lambda x: x[1]["recall"], reverse=True
        )
...
def get_compare_matrix_data():
    ...
    metrics = [list(matrix.data["metrics"][cat].values()) for cat in old_names], [
...
def tweet_annotation():
    ...
    if "cat" in request.args.to_dict().keys():
        ...
        tweet_table = sorted(
            [t for t in tweet_table.items()], key=lambda x: x[1]["id"], reverse=True
        )
    ...
    return render_template(
        "tweet_annotate.html", tweet_table=tweet_table, categories=categories, analysis=analysis
    )
...
def annotation_summary(analysis_id):
    ...
    alltag_table = sorted(
        [t for t in tagdict.items()], key=lambda x: x[1]["nr_tweets"], reverse=True
    )
...
def annotations():
    ...
    ann_dict = (
        analysis.annotation_counts(tweets, current_user.id)
        if not shared
        else analysis.annotation_counts(tweets, "all")
    )
...
def save_annotation():
    ...
    for l in loc_list:
        coords[l] = [
            coordinates["word_locs"][l],
            re.sub(r"[^\w\s]", "", coordinates["word_locs"][l].lower()),
        ]
    ...
    myanns = TweetAnnotation.query.filter(
        TweetAnnotation.tweet == tweet.id, TweetAnnotation.user == current_user.id
    ).all()
    my_tuples = nlp4all.utils.ann_create_css_info(
        mytagcounts, tweet.full_text, ann_tags, myanns
...
def draggable():
    ...
    print("hep2")
    tag = TweetTag(
        category=category.id, analysis=analysis.id, tweet=this_tweet.id, user=current_user.id
    )
    db.session.add(tag)
    db.session.commit()
