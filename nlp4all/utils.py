    ...
    for word in split_list:
        ...
        if clean_word in classifications:
            ...
            the_tup = (
                word,
                max_key,
                round(100 * classifications[clean_word][max_key]),
    ...
    for w in range(len(word_list)):
        ...
        if clean_word in classifications and sum(classifications[clean_word].values()) > 0:
            ...
            for m in ann:
                # print(str(w))
                if (
                    str(w) in m.coordinates["txt_coords"].keys()
                    ...
                    taglist = [
                        m.annotation_tag
                        for m in ann
                        if str(w) in m.coordinates["txt_coords"].keys()
                    ]
                    # print(taglist)
                    key_list = []
                    value_list = []
                    ...
                    max_key = max(classifications[clean_word].items(), key=operator.itemgetter(1))[
                        0
                    ]
                    ...
                    the_tup = (
                        ...
                        w,
                        m.annotation_tag,
                        classifications[clean_word][m.annotation_tag],
                        category_color_dict[m.annotation_tag],
                        value_list,
                        key_list,
                    )  # TODO: show all tags
                    if tups[w][2] == "none":
    ...
    for i in cat_names:
        ...
        if sum([matrix_classes[x] for x in recall_keys]) > 0:
            metrics[i]["recall"] = round(
                matrix_classes[tp_key] / sum([matrix_classes[x] for x in recall_keys]), 2
            )
        ...
        if sum([matrix_classes[x] for x in precision_keys]) > 0:
            metrics[i]["precision"] = round(
                matrix_classes[tp_key] / sum([matrix_classes[x] for x in precision_keys]), 2
            )
    return metrics
